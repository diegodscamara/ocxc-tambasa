...
export const TmbCheckoutContinueToPaymentButton = () => import('../index');

describe('TmbCheckoutContinueToPaymentButton renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutContinueToPaymentButton};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutContinueToPaymentButton works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutContinueToPaymentButton',
            regions: []
          }
        }
      }
