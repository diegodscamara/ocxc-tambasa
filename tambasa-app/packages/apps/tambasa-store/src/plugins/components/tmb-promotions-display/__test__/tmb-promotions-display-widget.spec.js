...
export const TmbPromotionsDisplay = () => import('../index');

describe('TmbPromotionsDisplay renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbPromotionsDisplay};
  beforeAll(async () => {
    /**
  ...
  it('TmbPromotionsDisplay works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbPromotionsDisplay',
            regions: []
          }
        }
      }
