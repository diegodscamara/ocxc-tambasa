...
export const TmbFacetedNavigation = () => import('../index');

describe('TmbFacetedNavigation renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbFacetedNavigation};
  beforeAll(async () => {
    /**
  ...
  it('TmbFacetedNavigation works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbFacetedNavigation',
            regions: []
          }
        }
      }
