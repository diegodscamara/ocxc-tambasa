...
export const TmbProfileUpdateExpiredPassword = () => import('../index');

describe('TmbProfileUpdateExpiredPassword renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProfileUpdateExpiredPassword};
  beforeAll(async () => {
    /**
  ...
  it('TmbProfileUpdateExpiredPassword works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProfileUpdateExpiredPassword',
            regions: []
          }
        }
      }
