...
export const TmbProductResultSKU = () => import('../index');

describe('TmbProductResultSKU renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProductResultSKU};
  beforeAll(async () => {
    /**
  ...
  it('TmbProductResultSKU works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProductResultSKU',
            regions: []
          }
        }
      }
