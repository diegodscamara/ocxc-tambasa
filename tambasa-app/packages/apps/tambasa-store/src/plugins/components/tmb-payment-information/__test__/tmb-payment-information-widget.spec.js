...
export const TmbPaymentInformation = () => import('../index');

describe('TmbPaymentInformation renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbPaymentInformation};
  beforeAll(async () => {
    /**
  ...
  it('TmbPaymentInformation works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbPaymentInformation',
            regions: []
          }
        }
      }
