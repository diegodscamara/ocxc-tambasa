...
export const TmbCheckoutBackToPreviousButton = () => import('../index');

describe('TmbCheckoutBackToPreviousButton renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutBackToPreviousButton};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutBackToPreviousButton works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutBackToPreviousButton',
            regions: []
          }
        }
      }
