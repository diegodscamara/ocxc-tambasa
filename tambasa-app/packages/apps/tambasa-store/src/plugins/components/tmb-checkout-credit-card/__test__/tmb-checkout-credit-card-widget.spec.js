...
export const TmbCheckoutCreditCard = () => import('../index');

describe('TmbCheckoutCreditCard renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutCreditCard};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutCreditCard works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutCreditCard',
            regions: []
          }
        }
      }
