...
const CheckoutSavedCards = props => {
  ...
  const setElementValidity = useCallback((element, validator) => {
    ...
    const errorContainer = document.querySelector('.CheckoutSavedCards__ErrorContainer')
        ...
        {isLoggedIn && savedCards.length > 0 ? (
          <ul id={`savedCardsList-${id}`} className="CheckoutSavedCards__List" ref={savedCardsListElementRef}>
            {savedCards.map(savedCard => (
              <li key={savedCard.savedCardId} 
              className={`CheckoutSavedCards__ListItem 
                ${ selectedPaymentType === PAYMENT_TYPE_CARD && selectedSavedCardId === savedCard.savedCardId
                  ? 'isSelected'
                  : ''
                }
              `}>
                <label className="CheckoutSavedCards__CvvLabel" htmlFor={`${id}${savedCard.savedCardId}`}>
