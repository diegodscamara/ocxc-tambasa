...
export const TmbPromotionCodeEntry = () => import('../index');

describe('TmbPromotionCodeEntry renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbPromotionCodeEntry};
  beforeAll(async () => {
    /**
  ...
  it('TmbPromotionCodeEntry works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbPromotionCodeEntry',
            regions: []
          }
        }
      }
