...
export const TmbCheckoutPaymentMethodsContainer = () => import('../index');

describe('TmbCheckoutPaymentMethodsContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutPaymentMethodsContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutPaymentMethodsContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutPaymentMethodsContainer',
            regions: []
          }
        }
      }
