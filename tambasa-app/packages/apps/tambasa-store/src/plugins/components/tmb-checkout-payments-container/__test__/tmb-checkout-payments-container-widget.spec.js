...
export const TmbCheckoutPaymentsContainer = () => import('../index');

describe('TmbCheckoutPaymentsContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutPaymentsContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutPaymentsContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutPaymentsContainer',
            regions: []
          }
        }
      }
