...
export const TmbAccountTypeSelector = () => import('../index');

describe('TmbAccountTypeSelector renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbAccountTypeSelector};
  beforeAll(async () => {
    /**
  ...
  it('TmbAccountTypeSelector works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbAccountTypeSelector',
            regions: []
          }
        }
      }
