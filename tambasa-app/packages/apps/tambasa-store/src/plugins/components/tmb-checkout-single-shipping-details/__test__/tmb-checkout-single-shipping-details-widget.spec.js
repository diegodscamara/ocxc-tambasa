...
export const TmbCheckoutSingleShippingDetails = () => import('../index');

describe('TmbCheckoutSingleShippingDetails renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutSingleShippingDetails};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutSingleShippingDetails works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutSingleShippingDetails',
            regions: []
          }
        }
      }
