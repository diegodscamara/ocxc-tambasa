...
export const TmbCheckoutContinueToReviewOrderButton = () => import('../index');

describe('TmbCheckoutContinueToReviewOrderButton renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutContinueToReviewOrderButton};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutContinueToReviewOrderButton works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutContinueToReviewOrderButton',
            regions: []
          }
        }
      }
