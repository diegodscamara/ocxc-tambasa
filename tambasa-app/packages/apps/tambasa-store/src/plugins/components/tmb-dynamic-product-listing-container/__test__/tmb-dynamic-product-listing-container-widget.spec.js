...
export const TmbDynamicProductListingContainer = () => import('../index');

describe('TmbDynamicProductListingContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbDynamicProductListingContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbDynamicProductListingContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbDynamicProductListingContainer',
            regions: []
          }
        }
      }
