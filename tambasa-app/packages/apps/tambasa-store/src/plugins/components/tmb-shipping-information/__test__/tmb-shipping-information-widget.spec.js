...
export const TmbShippingInformation = () => import('../index');

describe('TmbShippingInformation renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbShippingInformation};
  beforeAll(async () => {
    /**
  ...
  it('TmbShippingInformation works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbShippingInformation',
            regions: []
          }
        }
      }
