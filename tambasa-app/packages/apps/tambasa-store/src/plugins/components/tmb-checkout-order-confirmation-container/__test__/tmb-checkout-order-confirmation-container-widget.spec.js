...
export const TmbCheckoutOrderConfirmationContainer = () => import('../index');

describe('TmbCheckoutOrderConfirmationContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutOrderConfirmationContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutOrderConfirmationContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutOrderConfirmationContainer',
            regions: []
          }
        }
      }
