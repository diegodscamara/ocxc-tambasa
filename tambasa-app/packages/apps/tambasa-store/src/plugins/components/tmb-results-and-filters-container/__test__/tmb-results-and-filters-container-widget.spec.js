...
export const TmbResultsAndFiltersContainer = () => import('../index');

describe('TmbResultsAndFiltersContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbResultsAndFiltersContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbResultsAndFiltersContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbResultsAndFiltersContainer',
            regions: []
          }
        }
      }
