...
export const TmbProductListingSummaryInformation = () => import('../index');

describe('TmbProductListingSummaryInformation renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProductListingSummaryInformation};
  beforeAll(async () => {
    /**
  ...
  it('TmbProductListingSummaryInformation works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProductListingSummaryInformation',
            regions: []
          }
        }
      }
