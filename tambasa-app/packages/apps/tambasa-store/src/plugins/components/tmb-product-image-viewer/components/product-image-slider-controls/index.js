...
const ProductImageSliderControls = props => {
  ...
  return (
    <Styled id="TmbProductImageViewer__SliderControls" css={css}>
      <div
        className={`${mobile ? 'TmbProductImageViewer__SliderControls--Mobile' : 'TmbProductImageViewer__SliderControls'}`}
      >
        {images.length > 1 && (
          <ul className="TmbProductImageViewer__SliderIndicators">
            {images.map((value, index) => {
              ...
              return (
                <li key={imgKey}>
                  <button
                    className="TmbProductImageViewer__SliderIndicatorButton"
