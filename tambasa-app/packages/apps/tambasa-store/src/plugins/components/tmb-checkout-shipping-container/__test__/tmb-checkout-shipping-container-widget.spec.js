...
export const TmbCheckoutShippingContainer = () => import('../index');

describe('TmbCheckoutShippingContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutShippingContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutShippingContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutShippingContainer',
            regions: []
          }
        }
      }
