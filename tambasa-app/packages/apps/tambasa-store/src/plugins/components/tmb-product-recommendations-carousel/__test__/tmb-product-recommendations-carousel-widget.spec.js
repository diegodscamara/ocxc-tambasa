...
export const TmbProductRecommendationsCarousel = () => import('../index');

describe('TmbProductRecommendationsCarousel renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProductRecommendationsCarousel};
  beforeAll(async () => {
    /**
  ...
  it('TmbProductRecommendationsCarousel works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProductRecommendationsCarousel',
            regions: []
          }
        }
      }
