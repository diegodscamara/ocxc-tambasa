...
export const TmbHeaderMyAccount = () => import('../index');

describe('TmbHeaderMyAccount renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbHeaderMyAccount};
  beforeAll(async () => {
    /**
  ...
  it('TmbHeaderMyAccount works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbHeaderMyAccount',
            regions: []
          }
        }
      }
