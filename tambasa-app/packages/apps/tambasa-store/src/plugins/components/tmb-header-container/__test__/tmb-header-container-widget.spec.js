...
export const tmbHeaderContainer = () => import('../index');

describe('tmbHeaderContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {tmbHeaderContainer};
  beforeAll(async () => {
    /**
  ...
  it('tmbHeaderContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'tmbHeaderContainer',
            regions: []
          }
        }
      }
