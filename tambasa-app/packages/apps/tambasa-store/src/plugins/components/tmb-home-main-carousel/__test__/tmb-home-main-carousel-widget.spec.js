...
export const TmbHomeMainCarousel = () => import('../index');

describe('TmbHomeMainCarousel renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbHomeMainCarousel};
  beforeAll(async () => {
    /**
  ...
  it('TmbHomeMainCarousel works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbHomeMainCarousel',
            regions: []
          }
        }
      }
