...
export const TmbHeaderSubContainer = () => import('../index');

describe('TmbHeaderSubContainer renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbHeaderSubContainer};
  beforeAll(async () => {
    /**
  ...
  it('TmbHeaderSubContainer works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbHeaderSubContainer',
            regions: []
          }
        }
      }
