...
export const TmbSearchTermsSummary = () => import('../index');

describe('TmbSearchTermsSummary renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbSearchTermsSummary};
  beforeAll(async () => {
    /**
  ...
  it('TmbSearchTermsSummary works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbSearchTermsSummary',
            regions: []
          }
        }
      }
