...
export const TmbCheckoutProgressTracker = () => import('../index');

describe('TmbCheckoutProgressTracker renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCheckoutProgressTracker};
  beforeAll(async () => {
    /**
  ...
  it('TmbCheckoutProgressTracker works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCheckoutProgressTracker',
            regions: []
          }
        }
      }
