...
export const TmbViewOrdersButton = () => import('../index');

describe('TmbViewOrdersButton renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbViewOrdersButton};
  beforeAll(async () => {
    /**
  ...
  it('TmbViewOrdersButton works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbViewOrdersButton',
            regions: []
          }
        }
      }
