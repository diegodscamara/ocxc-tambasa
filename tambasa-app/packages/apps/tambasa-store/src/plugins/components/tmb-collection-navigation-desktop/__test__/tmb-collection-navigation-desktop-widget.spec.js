...
export const TmbCollectionNavigationDesktop = () => import('../index');

describe('TmbCollectionNavigationDesktop renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCollectionNavigationDesktop};
  beforeAll(async () => {
    /**
  ...
  it('TmbCollectionNavigationDesktop works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCollectionNavigationDesktop',
            regions: []
          }
        }
      }
