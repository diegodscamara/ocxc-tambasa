...
export const TmbCartOrderSummary = () => import('../index');

describe('TmbCartOrderSummary renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCartOrderSummary};
  beforeAll(async () => {
    /**
  ...
  it('TmbCartOrderSummary works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCartOrderSummary',
            regions: []
          }
        }
      }
