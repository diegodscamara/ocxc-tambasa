...
export const TmbDynamicProductList = () => import('../index');

describe('TmbDynamicProductList renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbDynamicProductList};
  beforeAll(async () => {
    /**
  ...
  it('TmbDynamicProductList works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbDynamicProductList',
            regions: []
          }
        }
      }
