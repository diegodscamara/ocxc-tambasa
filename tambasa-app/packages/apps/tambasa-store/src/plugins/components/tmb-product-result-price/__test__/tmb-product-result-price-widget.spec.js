...
export const TmbProductResultPrice = () => import('../index');

describe('TmbProductResultPrice renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProductResultPrice};
  beforeAll(async () => {
    /**
  ...
  it('TmbProductResultPrice works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProductResultPrice',
            regions: []
          }
        }
      }
