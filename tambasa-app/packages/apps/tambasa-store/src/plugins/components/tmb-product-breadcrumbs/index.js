  ...
  * TmbProductBreadcrumbs are applied to product, collections and search results pages.
  *
  * @param props
  */
 const TmbProductBreadcrumbs = props => {
   ...
   if (searchPageId) {
     ...
     if (previousBreadcrumbs && previousBreadcrumbs.searchCrumbs && previousBreadcrumbs.searchCrumbs.length > 0) {
       return (
         <Styled id="TmbProductBreadcrumbs" css={css}>
           <div className="TmbProductBreadcrumbs">
             <Link className="TmbProductBreadcrumbs__BreadcrumbLink" href=".">
               {t(textHome)}
             </Link>
             {divider}
             {previousBreadcrumbs.searchCrumbs.map(crumbData => (
               <Link
                 className="TmbProductBreadcrumbs__BreadcrumbLink"
               ...
               >
                 ...
                 >
                   <span className={crumbData.correctedTerms && 'TmbProductBreadcrumbs__CorrectedLabel'}>
                     &quot;{crumbData.terms}&quot;
                   </span>
         ...
         <div className="TmbProductBreadcrumbs">
           ...
           {parentCategories.map(category => (
             <span key={category.id}>
               {divider}
               <Link className="TmbProductBreadcrumbs__BreadcrumbLink" href={category.route}>
   ...
   return <Styled id="TmbProductBreadcrumbs" css={css} />;
 };
 
 TmbProductBreadcrumbs.propTypes = {
   /**
 ...
 TmbProductBreadcrumbs.defaultProps = {
   breadcrumbs: {},
   pages: {},
   productId: '',
   products: {}
 ...
 export default connect(getPageData)(TmbProductBreadcrumbs);
