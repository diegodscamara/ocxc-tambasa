...
export const TmbProfileRegistration = () => import('../index');

describe('TmbProfileRegistration renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbProfileRegistration};
  beforeAll(async () => {
    /**
  ...
  it('TmbProfileRegistration works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbProfileRegistration',
            regions: []
          }
        }
      }
