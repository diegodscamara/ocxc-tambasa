...
export const tmbHomeImageBanner = () => import('../index');

describe('tmbHomeImageBanner renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {tmbHomeImageBanner};
  beforeAll(async () => {
    /**
  ...
  it('tmbHomeImageBanner works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'tmbHomeImageBanner',
            regions: []
          }
        }
      }
