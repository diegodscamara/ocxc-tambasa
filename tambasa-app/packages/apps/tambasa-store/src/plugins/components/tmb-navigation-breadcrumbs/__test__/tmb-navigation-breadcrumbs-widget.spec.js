...
export const TmbNavigationBreadcrumbs = () => import('../index');

describe('TmbNavigationBreadcrumbs renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbNavigationBreadcrumbs};
  beforeAll(async () => {
    /**
  ...
  it('TmbNavigationBreadcrumbs works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbNavigationBreadcrumbs',
            regions: []
          }
        }
      }
