...
export const TmbCartItemDetails = () => import('../index');

describe('TmbCartItemDetails renders correctly and shows after changing visible state', () => {
  /**
   * Reference to dynamic imported comps to resolve and preload.
   */
  const comps = {TmbCartItemDetails};
  beforeAll(async () => {
    /**
  ...
  it('TmbCartItemDetails works as expected', async done => {
    /**
    ...
    const state = {
      pageRepository: {
        widgets: {
          dsWidgetInstance: {
            id: 'dsWidgetInstance',
            componentId: 'TmbCartItemDetails',
            regions: []
          }
        }
      }
