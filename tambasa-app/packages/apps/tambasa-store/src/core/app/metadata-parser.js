...
const getComponents = (meta, appName) => {
  ...
  const pathPackageJson = '@oracle-cx-commerce' + '/' + appName + "/package.json"; 
  ...
  Object.keys(aliases).forEach(keyPath => {
    ...
    if(keyPath.indexOf('@oracle-cx-commerce/styles') < 0 ) {
  ...
  const getComponentDocs = () => {
    const componentPath = `${path.dirname(require.resolve('@oracle-cx-commerce/react-components/package.json'))}`;
...
const getStyles = (meta, appName) => {
  ...
  */
  ...
  const coreStylesPath = `${path.dirname(require.resolve('@oracle-cx-commerce/styles'))}`;
  ...
  Object.keys(aliases).forEach(keyPath => {
    ...
    if( CoreStyleDocsKeysArray.indexOf(key) >= 0 ) {
      CoreStylesDocs[key].widgetStyle = fs.existsSync( require.resolve(aliases[keyPath])) ? fs.readFileSync(require.resolve(aliases[keyPath]), 'utf8') : '';
    }
  });
